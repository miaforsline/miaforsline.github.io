...
In California from 2013 - 2021, does air quality (as measured by PM2.5 concentrations) vary with poverty rates (as measured by the percent of the population living below two times the federal poverty level)?
...
```{r, fig.cap = '2013 - 2021 mean PM2.5 in California was not normally distributed. Annual mean concentrations of PM2.5 (µg/m3) per census tract ranged from 1.65 to 21.2. Data was sourced from CalEnviroScreen 1.1 - 4.0 (https://oehha.ca.gov/).'}
...
sub %>% 
  ...
  gt() %>%  
  tab_header(
    title = "Table 1. Mean and standard deviation of mean PM2.5 (ug/m3) in California", 
    subtitle = "Data from CalEnviroScreen (https://oehha.ca.gov/).")
```
...
```{r, fig.cap = '2013 - 2021 poverty rates in California were not normally distributed. The percent of the population per census tract living below two times the federal poverty level ranged from 0% to 99%. Data was sourced from CalEnviroScreen 1.1 - 4.0 (https://oehha.ca.gov/).'}
...
sub %>% 
  ...
  gt() %>%  
  tab_header(
    title = "Table 2. Mean and standard deviation of poverty rates in California (%)", 
    subtitle = "Data from CalEnviroScreen (https://oehha.ca.gov/)."
  )
```
...
For all time periods, annual mean PM2.5 concentrations were significantly influenced by the percent of people living below twice the federal poverty level. In 2013, PM2.5 increased by 0.035 µg/m3 as the poverty rate increased by 1% (p-value = `r sum1$coefficients[8]`, Table \@ref(tab:2013)). 
...
mod1 %>%
  summary() %>%
  xtable() %>%
  kable(caption = "2013 Poverty rate significantly influences air quality",
        digits = 3) %>% 
  kable_styling(latex_options = "HOLD_position")
```

In 2014, PM2.5 increased by 0.028 µg/m3 as the poverty rate increased by 1% (p-value = `r sum2$coefficients[8]`, Table \@ref(tab:2014)). 
...
mod2 %>%
  summary() %>%
  xtable() %>%
  kable(caption = "2014 Poverty rate significantly influences air quality", 
        digits = 3) %>% 
  kable_styling(latex_options = "HOLD_position")
```

In 2018, PM2.5 increased by 0.030 µg/m3 as the poverty rate increased by 1% (p-value = `r sum3$coefficients[8]`, Table \@ref(tab:2018)).
...
mod3 %>%
  summary() %>%
  xtable() %>%
  kable(caption = "2018 Poverty rate significantly influences air quality",
        digits = 3) %>% 
  kable_styling(latex_options = "HOLD_position")
```

In 2021, PM2.5 increased by 0.029 µg/m3 as the poverty rate increased by 1% (p-value = `r sum4$coefficients[8]`, Table \@ref(tab:2021)).
...
mod4 %>%
  summary() %>% 
  xtable() %>% 
  kable(caption = "2021 Poverty rate significantly influences air quality",
        digits = 3) %>% 
  kable_styling(latex_options = "HOLD_position")
```
...
Ultimately, I found that---in California during 2013, 2014, 2018, and 2021---air quality significantly associated with poverty rates. Specifically, for all four years, annual mean concentrations of PM2.5 (µg/m^3^) increased as the percent of people living below twice the federal poverty level increased (Figure \@ref(fig:final)). In other words, air quality was lower in census tracts with higher poverty rates.

These findings support my hypothesis that mean PM2.5 and poverty in California are related.
