...
The environmental burden of poor air quality is not shared equally across the state. For example, the San Joaquin Valley's economically disadvantaged and ethnically diverse communities breathe some of the most polluted air in the nation (Buckley 2017). As a result, vulnerable communities such as Mexican American immigrant farm workers and their families experience disproportionately high rates of asthma attacks, hospital admissions, and other medical issues (Schwartz and Pepper 2009). This inequitable pattern is well recognized in other states (Qian and Wu 2019), the United States overall (Tessum et al. 2021), and even other countries (Li et al. 2018).
...
For all time periods, annual mean PM2.5 concentrations were significantly influenced by the percent of people living below twice the federal poverty level. In 2013, PM2.5 increased by 0.035 µg/m3 as the poverty rate increased by 1% (p-value = `r sum1$coefficients[8]`, sd = `r sum1$coefficients[4]`). In 2014, PM2.5 increased by 0.028 µg/m3 as the poverty rate increased by 1% (p-value = `r sum2$coefficients[8]`, sd = `r sum2$coefficients[4]`). In 2018, PM2.5 increased by 0.030 µg/m3 as the poverty rate increased by 1% (p-value = `r sum3$coefficients[8]`, sd = `r sum3$coefficients[4]`). In 2021, PM2.5 increased by 0.029 µg/m3 as the poverty rate increased by 1% (p-value = `r sum4$coefficients[8]`, sd = `r sum4$coefficients[4]`).
...
As expected, I found---in California during 2013, 2014, 2018, and 2021---a statistically significant relationship between air quality and poverty rates. Specifically, for all four years, annual mean concentrations of PM2.5 (µg/m^3^) increased as the percent of people living below twice the federal poverty level increased (Figure \@ref(fig:final)). In other words, air quality was lower in census tracts with higher poverty rates.

These findings supported my hypothesis and corroborated prior research that has identified PM2.5 disparities based on socioeconomic factors in California (Mousavi et al. 2021)
...
A crucial next step is to explore the biological significance of these findings. While the models returned statistically significant results, it is important to determine if a `r sum1$coefficients[2]` µg/m^~3~^, `r sum2$coefficients[2]` µg/m^~3~^, `r sum3$coefficients[2]` µg/m^~3~^, or `r sum4$coefficients[2]` µg/m^~3~^ change in PM2.5 per 1% increase in poverty is a large change enough to actually harm communities.
...
Full code can be found [here](https://github.com/miaforsline/miaforsline.github.io/tree/main/_posts/2021-11-18-calenviroscreen).
