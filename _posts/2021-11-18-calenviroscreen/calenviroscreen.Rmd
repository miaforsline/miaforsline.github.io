...
```{r}
caption = str_wrap("Figure 1. Histograms of 2013 - 2021 mean concentrations of PM2.5 in California. Mean concentration values ranged from 1.65 to 21.2. Data was not normally distributed for any year. Data was sourced from CalEnviroScreen 1.1 - 4.0 (https://oehha.ca.gov/).", 100)
...
ggplot(data = joined, aes(x = PM2.5)) + 
  ...
  labs(x = expression(paste("Annual mean concentration of PM2.5 per census tract (µg/m"^3~")")), 
       y = "Frequency",
       caption = caption)
```
...
```{r}
caption = str_wrap("Figure 2. Histograms of 2013 - 2021 poverty rates in California. Poverty rates ranged from 0% to 99%. Data was not normally distributed for any year. Data was sourced from CalEnviroScreen 1.1 - 4.0 (https://oehha.ca.gov/).", 100)
...
ggplot(data = joined, aes(x = Poverty)) + 
  ...
  theme_classic() + 
  facet_wrap(~Year, ncol = 1) + 
  labs(x = "% of population per census tract living below two times the federal poverty level", 
       y = "Frequency",
       caption = caption)
```
...
sub %>% 
  ...
  gt() %>%  
  tab_header(
    title = "% of census tract population living below two times the federal poverty level in California", 
    subtitle = "Data from CalEnviroScreen (https://oehha.ca.gov/)."
  )
```
...
mod1 %>%
  summary() %>%
  xtable() %>%
  kable(caption = "2013 Poverty Rate Positively Influences Air Quality",
        digits = 3) %>% 
...
mod2 %>%
  summary() %>%
  xtable() %>%
  kable(caption = "2014 Poverty Rate Positively Influences Air Quality", 
        digits = 3) %>% 
  kable_styling(latex_options = "HOLD_position")
...
mod3 %>%
  summary() %>%
  xtable() %>%
  kable(caption = "2018 Poverty Rate Positively Influences Air Quality",
        digits = 3) %>% 
  kable_styling(latex_options = "HOLD_position")
...
mod4 %>%
  summary() %>% 
  xtable() %>% 
  kable(caption = "2021 Poverty Rate Positively Influences Air Quality",
        digits = 3) %>% 
  kable_styling(latex_options = "HOLD_position")
```
...
ggplot(data = joined, aes(x = Poverty, y = PM2.5)) +
  ...
  labs(x = "Poverty Rate Per Census Tract (%)",
       y = expression(paste
                       ("Mean PM2.5 Per Census Tract (µg/m"^3~")")),
       caption = "Caption") +
