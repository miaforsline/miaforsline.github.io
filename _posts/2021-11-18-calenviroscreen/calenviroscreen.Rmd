...
I downloaded 2013 - 2021 CalEnviroScreen (CES) data from the [[California Office of Environmental Health Hazard Assessment (OEHHA)]{.ul}](https://oehha.ca.gov/)and the [[California Open Data Portal]{.ul}](https://data.ca.gov/):

-   [[Versions 1.1 (Sept 2013)]{.ul}](https://oehha.ca.gov/calenviroscreen/report-general-info/calenviroscreen-11)
-   [[Versions 2.0 (Oct 2014)]{.ul}](https://oehha.ca.gov/calenviroscreen/report/calenviroscreen-version-20)
-   [[Versions 3.0 (June 2018)]{.ul}](https://data.ca.gov/dataset/calenviroscreen-3-0-results)
-   [[Versions 4.0 (Oct 2021)]{.ul}](https://oehha.ca.gov/calenviroscreen/report/calenviroscreen-40)

Each CES data set contains columns of environmental pollution burden indicators, including exposure indicators such as PM2.5, and population characteristics, including socioeconomic factor indicators such as rates of poverty. Each census tract in California is represented as a row and assigned a value per environmental indicator and population characteristic.

Out of the myriad components, I am interested in:

### 1. **PM2.5**

The annual mean concentration of PM2.5 is a weighted average of measured monitor concentrations and satellite observations (ug/m^3^) over 3 years to avoid account for uneven sampling frequency. For example, the CES 1.1. report used data from 2007 - 2009 while the CES 4.0 report used 2015 - 2017. All reports used data from the [[California Air Resources Board's]{.ul}](https://ww2.arb.ca.gov/homepage) Air Monitoring Network (AMN). In addition to the AMN, CES 3.0 and CES 4.0 incorporated [[Satellite Remote Sensing Data]{.ul}](https://ww2.arb.ca.gov/resources/documents/air-quality-research-using-satellite-remote-sensing).

Data were more likely to be high resolution around certain cities or localized areas, and not all cities have air monitoring stations. Locales with little to no data were either omitted or estimated using nearby locations' data. For example, in CES 1.1, census tracts with centers \> 50km away from the nearest air monitor were omitted. In CES 4.0, missing data was estimated using regression relationships with nearby sites.
...
The quarterly estimates were then averaged to calculate annual means (Figure 3).
...
For all time periods, annual mean PM2.5 concentrations were significantly influenced by the percent of people living below twice the federal poverty level. In 2013, PM2.5 increased by 0.035 µg/m3 as the poverty rate increased by 1% (p-value \< 0.001, sd = `r sum1$coefficients[4]`). In 2014, PM2.5 increased by 0.028 µg/m3 as the poverty rate increased by 1% (p-value \< 0.001, sd = `r sum2$coefficients[4]`). In 2018, PM2.5 increased by 0.030 µg/m3 as the poverty rate increased by 1% (p-value \< 0.001, sd = `r sum3$coefficients[4]`). In 2021, PM2.5 increased by 0.029 µg/m3 as the poverty rate increased by 1% (p-value \< 0.001, sd = `r sum4$coefficients[4]`). These results support my hypothesis that mean PM2.5 and poverty in California are significantly related (Figure 3). More specifically, over time, the relationship between mean PM2.5 and poverty rate has slightly changed but remained fairly similar.
...
The full code can be accessed [[here]{.ul}](https://github.com/miaforsline/miaforsline.github.io/tree/main/_posts/2021-11-18-calenviroscreen).
